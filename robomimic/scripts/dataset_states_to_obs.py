...
def extract_trajectory(
    ...
    if add_ll_info:
        xpos = env.env.sim.data.get_site_xpos(env.env.nuts[0].important_sites["handle"])
        xmat = env.env.sim.data.get_site_xmat(env.env.nuts[0].important_sites["handle"])
        obs["handle_pos"] = xpos
        obs["handle_mat"] = xmat
        obs["handle_quat"] = T.mat2quat(xmat)
        obs["handle_angle"] = T.quat2axisangle(obs["handle_quat"])

        # Adding peg
        xpos = np.array(env.env.sim.data.body_xpos[env.env.peg1_body_id])
        xmat = np.array(env.env.sim.data.body_xmat[env.env.peg1_body_id]).reshape(3,3)
        obs["peg_pos"] = xpos
        obs["peg_mat"] = xmat
        obs["peg_quat"] = T.mat2quat(xmat)
        obs["peg_angle"] = T.quat2axisangle(obs["peg_quat"])
    ...
    for t in range(1, traj_len + 1):
        ...
        if add_ll_info:
            ...
            xpos = env.env.sim.data.get_site_xpos(env.env.nuts[0].important_sites["handle"])
            xmat = env.env.sim.data.get_site_xmat(env.env.nuts[0].important_sites["handle"])
            next_obs["handle_pos"] = xpos
            next_obs["handle_mat"] = xmat
            next_obs["handle_quat"] = T.mat2quat(xmat)
            next_obs["handle_angle"] = T.quat2axisangle(next_obs["handle_quat"])

            # Adding peg
            xpos = np.array(env.env.sim.data.body_xpos[env.env.peg1_body_id])
            xmat = np.array(env.env.sim.data.body_xmat[env.env.peg1_body_id]).reshape(3,3)
            next_obs["peg_pos"] = xpos
            next_obs["peg_mat"] = xmat
            next_obs["peg_quat"] = T.mat2quat(xmat)
            next_obs["peg_angle"] = T.quat2axisangle(next_obs["peg_quat"])
...
def dataset_states_to_obs(args):
    ...
    for ind in range(len(demos)):
        ...
        if "data/{}/error_detected".format(ep) in f:
            traj["error_detected"] = f["data/{}/error_detected".format(ep)][()]
        else:
            traj["error_detected"] = -1 * np.ones(len(actions))

        for k in f["data/{}".format(ep)].keys():
            if k not in traj:
                print("adding original key {} to traj".format(k))
                traj[k] = f["data/{}/{}".format(ep, k)][()]
        ...
        ep_data_grp.create_dataset("error_detected", data=np.array(traj["error_detected"]))
        ...
        for k in traj.keys():
            if k not in ep_data_grp:
                if k in ["next_obs", "initial_state_dict"]:
                    continue
                print("adding original key {} to data".format(k))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--add_ll_info",
        action="store_true",
        help="adding low-level info (handle, peg pose)"
    )
