...
def playback_trajectory_with_env(
    ...
    video_writer = imageio.get_writer(video_writer_path + "/{}.mp4".format(idx), fps=20)
...
def playback_trajectory_plot_data(
    env, 
    initial_state, 
    states,
    obs,
    ...
    error_state=None,
    actions=None,
    original_video_writer_path=None,
    video_writer_path=None,
    camera_names=None,
    idx=None):
    
    traj_video = cv2.VideoCapture(original_video_writer_path + '/{}.mp4'.format(idx))
    video_writer = imageio.get_writer(video_writer_path + "/{}_labeled.mp4".format(idx), fps=20)
    
    video_lst = []
    while(traj_video.isOpened()):
        ret1, frame1 = traj_video.read()
        assert ret1
        video_lst.append(frame1)
    ...
    if isinstance(detector, BCDreamer_Failure):
        ...
        for i in range(traj_len):
            ...
            plt.plot(plot_data_prob[:i+1], label="recons_loss")
        ...
        for i in range(traj_len):
            ...
            plt.ylim(0, ymax)
            plt.xlim(0, detector.horizon())
            plt.plot(plot_data_current[:i+1], label="current")
            plt.plot(plot_data_imagine[:i+1], label="imagine")
            plt.savefig("tmp_{}.png".format(unique_str))
            plt.clf()
            plot_image = cv2.imread("tmp_{}.png".format(unique_str))
            plot_images.append(plot_image)
        ...
        for i in range(len(video_lst)):
            video_img = video_lst[i]
            plot_image = plot_images[i]
            is_novel = detector.above_threshold(plot_data[i])
            if is_novel:
                video_img = apply_filter(video_img, color=(255, 0, 0))
            video_img = np.concatenate([video_img, plot_image], axis=1) # concatenate horizontally
            video_writer.append_data(video_img)
    ...
    for ind in range(len(demos)):
        ...
        if args.detector_type == "bc_dreamer_ood":
            ...
            if not args.use_prob:
                error_label = np.load(args.error_state_dir + "/rewards_{}.npy".format(ind))
                error_probs = np.load(args.error_state_dir + "/rewards_probs{}.npy".format(ind))
        ...
        if args.is_first:
            ...
            playback_trajectory_with_env(
                ...
                video_writer_path=args.video_path,
                camera_names=args.render_image_names,
                idx=ind,
            )
        else:
            assert os.path.exists(args.video_path) # confirm video exists
            
            os.makedirs(os.path.dirname(args.label_video_path), exist_ok=True)
            
            playback_trajectory_plot_data(
                env=env, 
                initial_state=initial_state, 
                states=states,
                obs=obs,
                ...
                actions=actions,
                error_state=error_state,
                original_video_writer_path=args.label_video_path,
                video_writer_path=args.label_video_path,
                camera_names=args.render_image_names,
                idx=ind,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(optional) render trajectories with plot labels",
    )
    ...
    parser.add_argument(
        "--detector_type",
        type=str,
        choices=["momart", "ensemble", "vae_action", "bc_dreamer_ood", "bc_dreamer_failure"]
    )
