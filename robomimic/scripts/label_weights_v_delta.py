...
def count_weights_with_env(
    ...
    for i in range(traj_len):
        ...
        v_delta = algo.get_v_value(obs_dict=ob).item()
        
        adv_weights.append(v_delta)
   
    adv_weights = np.array(adv_weights[1:]) - np.array(adv_weights[:-1])
    adv_weights = np.concatenate([[0.], adv_weights])
    
    beta = algo.algo_config.adv.beta
    weights = np.exp(adv_weights / beta)
    if algo.algo_config.adv.use_final_clip is True:
...
if __name__ == "__main__":
    ...
    for ind in range(len(demos)):
        ...
        v_delta_weights = count_weights_with_env(
            env=env,
        ...
        print(v_delta_weights)

        ep_data_group = f["data/{}".format(ep)]
        ep_data_group.create_dataset("v_delta_weights", data=v_delta_weights)
