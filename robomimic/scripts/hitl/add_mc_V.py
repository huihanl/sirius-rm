...
def get_Q_traj(rewards, terminals):
    q = [0] * len(rewards)
    q[-1] = rewards[-1]
    for i in range(2, len(rewards)):
        idx = -i
        q[idx] = rewards[idx] + (1 - terminals[idx]) * gamma * q[idx + 1]
    return q
...
if __name__ == "__main__":
    ...
    for ep in demos:
        ...
        q_values = get_Q_traj(rewards, terminals)
        q_values_lst.append(q_values)

        ep_data_grp.create_dataset("weights", data=q_values)
    ...
    for q in q_values_lst[100:110]:
        plt.plot(np.exp(np.array(q) * 2))
    fig.savefig("sample_q.png")
