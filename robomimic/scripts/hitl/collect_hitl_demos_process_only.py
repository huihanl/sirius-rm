    ...
    def get_action(self, obs):
        ...
        for k in di:
            if "image" in k:
                ...
                diff_camera_height = 84
                diff_camera_width = 84
                if k == "robot0_eye_in_hand_image":
                    h = di[k].shape[0]
                    w = di[k].shape[1]
                    di[k] = cv2.resize(di[k],
                                       (0,0),
                                       fx=float(diff_camera_height) / h,
                                       fy=float(diff_camera_width) / w,
                                       )
                elif k == "agentview_image":
                    di[k] = di[k][60:, 30:210, :]

                ret[k] = ObsUtils.process_obs(di[k], obs_modality='rgb')
                """
        obs.update(ret)
...
def collect_trajectory_with_playback(
    ...
    while True:
        ...
        time.sleep(0.03)
        """
        if_sleep = args.sleep_time > 0 #and (not i < 80) #(i < 80 or (i > 120 and i < 200))
...
if __name__ == "__main__":
    ...
    assert os.path.exists(args.playback_dataset)
    assert os.path.exists(args.merge_with)
    ...
    if args.playback_dataset is not None:
        ...
        while True:
            ...
            if discard_traj:
                ...
                num_traj_saved += 1
                print("current intv samples: ", total_human_samples)
