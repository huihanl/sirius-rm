...
if [ ${PARTITION} == "titans" ]
then
  EXCLUDE="titan-3" #"titan-5,titan-12"                                          # Specific machines to avoid using
else
...
do
  if [ ${env}  == "coffee_v0" ]
    then 
      INTERPRETER="new-coffee"
  fi
done
      # Which conda / venv to use for sbatch script

# sbatch variables (per script)
MAX_HRS=100                                                          # Max hours this script is allowed to run
