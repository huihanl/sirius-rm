...
def playback_trajectory_with_env(
    ...
    language_corrections = [""] * traj_len

    if (intv_labels[()] != -10).all():
        return language_corrections
    ...
    while True:
        ...
        # last_grasp = 0
        # updated_actions, updated_labels = [], []
        ...
        for i in range(traj_len):
            ...
            if intv_labels is not None:
                if (i + 4 < traj_len and intv_labels[i+4] == -10) or intv_labels[i] == -10:
                    print('\033[91m ############# PRE-INTERVENTION ############# \033[0m')
                    if intv_labels[i-1] != -10:
                        time.sleep(0.03)
                    time.sleep(0.03)
                elif intv_labels[i-1] == -10 and intv_labels[i] != -10:
                    print('\033[92m  START HUMAN INTERVENTION  \033[0m')
                    language_correction = input("Enter language correction: ")

                    print()
                    print("\033[92m  Start relabeling from step {}  \033[0m".format(i))
                    # go back to relabel the pre-intervention part
                    for k in range(i-1, -1, -1):
                        if intv_labels[k] != -10:
                            print("\033[92m  Relabeling finished at step {}  \033[0m".format(k))
                            break
                        language_corrections[k] = language_correction
                    print()
                    print()

            # # Get the newest action
            # human_action, grasp = input2action(
            #     device=device,
            ...
            # human_action, last_grasp = post_process_spacemouse_action(human_action, grasp, last_grasp)
            # if not is_empty_input_spacemouse(human_action):
            #     time.sleep(0.03)
            #     print('\033[92m  Human Relabeling  \033[0m')
            ...
            # if intv_labels is not None:
            #     if is_empty_input_spacemouse(human_action) or intv_labels[i] != -10:
            #         updated_actions.append(actions[i])
            #         updated_labels.append(intv_labels[i])
            #     else:        
            #         updated_actions.append(human_action)
            #         updated_labels.append(20)
            # else:
            #     if is_empty_input_spacemouse(human_action):
            #         updated_actions.append(actions[i])
        ...
        user_final = ""
        while user_final != "y" and user_final != "n":
            user_final = input("Satisfied? (y/n): ")
        failed = user_final == "n"
...
def playback_dataset(args):
    ...
    try:
        language_correction_dict = np.load("language_correction_dict.npy", allow_pickle=True).item()
    except:
        language_correction_dict = {}

    for ep in demos:
        if ep in language_correction_dict:
            continue
        ...
        language_correction = playback_trajectory_with_env(
            env=env, 
        ...
        # store language correction
        language_correction_dict[ep] = language_correction
        print(language_correction_dict)
        np.save("language_correction_dict.npy", language_correction_dict)

        # ep_data_grp = f["data/{}".format(ep)]

        # if "language_correction" in ep_data_grp:
        #     del ep_data_grp["language_correction"] 
        
        # str_data_type = h5py.special_dtype(vlen=str)
        # ep_data_grp.create_dataset("language_correction", dtype=str_data_type, data=language_correction)
        # print("Language Correction:")
        # print(ep_data_grp["language_correction"][()])
