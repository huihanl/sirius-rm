...
INTV_STRS = {-1: "Demos", 0: "Rollout", 1: "Human", -10: "Mistake"}
INTV_COLORS = {-1: "white", 0: "yellow", 1: "green", -10: "red"}
        ...
        if write_video:
            if video_count % video_skip == 0:
                ...
                for cam_name in camera_names:
                    ...
                    text = INTV_STRS[intv_label]
                    proc_img = VisUtils.write_text_on_image(proc_img, text, def_color=INTV_COLORS[intv_label])
...
def playback_trajectory_with_obs(
    ...
    for i in range(traj_len):
        ...
        if video_count % video_skip == 0:
            intv_label = traj_grp["intv_labels"][i]
            # concatenate image obs together
            im = [traj_grp["obs/{}".format(k)][i] for k in image_names]
            frame = np.concatenate(im, axis=1)
            text = INTV_STRS[intv_label]
            frame = VisUtils.write_text_on_image(frame, text, def_color=INTV_COLORS[intv_label])
