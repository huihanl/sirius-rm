...
def algo_factory(algo_name, config, obs_key_shapes, ac_dim, device, batch_size=None, seq_length=None):
    """
    ...
    # add batch size and sequence length to algo kwargs if they are specified
    if batch_size is not None:
        algo_kwargs["batch_size"] = batch_size
    if seq_length is not None:
        algo_kwargs["seq_length"] = seq_length
...
class Algo(object):
    ...
    def __init__(
        ...
        # batch size and sequence length are optional, but if specified, they should be
        # passed to the constructor and stored in the algo object
        if batch_size is not None:
    ...
    def _update_optimizers(self):
        """
        Update new optimizers using @self.optim_params and places them into @self.optimizers.
        """
        for k in self.optim_params:
            # only make optimizers for networks that have been created - @optim_params may have more
            # settings for unused networks
            if k in self.nets and k not in self.optimizers:
                if isinstance(self.nets[k], nn.ModuleList):
                    self.optimizers[k] = [
                        TorchUtils.optimizer_from_optim_params(net_optim_params=self.optim_params[k], net=self.nets[k][i])
                        for i in range(len(self.nets[k]))
...
class PolicyAlgo(Algo):
    ...
    """
    def get_action(self, obs_dict, action_given=None, goal_dict=None):
        """
...
class RolloutPolicy(object):
    ...
    def start_episode(self, eval_policy_only=True):
        """
    ...
    def __call__(self, ob, action=None, goal=None, return_embedding=False):
        """
        ...
        if not return_embedding:
            ...
            res = self.policy.get_action(obs_dict=ob, goal_dict=goal)
            ...
            ac, embedding = self.policy.get_action(obs_dict=ob, goal_dict=goal)
            return TensorUtils.to_numpy(ac[0]), TensorUtils.to_numpy(embedding[0])
