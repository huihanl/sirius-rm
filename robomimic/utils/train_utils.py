...
# from utils.parallel_env_wrapper import ParallelEnvWrapper
#from stable_baselines3.common.vec_env import SubprocVecEnv

from robomimic.utils.dataset import (
    SequenceDataset, BootstrapDataset, IWRDataset, WeightedDataset,
    ClassifierRelabeledDataset, PreintvRelabeledDataset,
    NoveltyRelabeledDataset,
    WeightedDatasetClassifier
)
...
def get_exp_dir(config, base_base_dir, auto_remove_exp_dir=False, ):
    """
    ...
    if not os.path.isabs(base_output_dir):
        ...
        base_output_dir = os.path.join(base_base_dir, base_output_dir)
...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None):
    ...
    if "dyn" in config.algo and config.algo.dyn.use_reward:
        if config.algo.dyn.rew.use_weighted_loss: # use weighted loss
            ds_kwargs.update(
                use_sampler=True,
                classifier_sampler=None,
                classifier_use_weighted_loss=True,
            )
        else: # use weighted sampling 
            # update bc_dreamer classifier sampler
            if config.algo.dyn.rew.rew_class == "binary":
                ds_kwargs.update(
                    use_sampler=True,
                    classifier_sampler="2class",
                )
            elif config.algo.dyn.rew.rew_class == "three_class":
                ds_kwargs.update(
                    use_sampler=True,
                    classifier_sampler="3class",
                )

    if config.algo.remove_intervention_sampler:
        ds_kwargs.update(
            remove_intervention_sampler=True,
        )

    if not config.algo.hc_weights.enabled and not config.train.classifier_relabeling.enabled and \
    ...
    if not config.train.use_weighted_sampling:
        ds_kwargs.update(
            ...
            ours_percentage=hc_config.ours_percentage,
            diff_weights_diff_rounds=hc_config.diff_weights_diff_rounds,
            rounds_key=hc_config.rounds_key,
            round_upweights=hc_config.round_upweights,
            normalize_after_round_upweight=hc_config.normalize_after_round_upweight,
            rounds_resampling=hc_config.rounds_resampling,
            resampling_weight=hc_config.resampling_weight,
    ...
    if config.train.preintv_relabeling.enabled:
        assert not config.train.use_weighted_sampling  # just for classifier training

    if config.train.use_weighted_sampling:
        return WeightedDatasetClassifier(traj_label_type=hc_config.traj_label_type, **ds_kwargs)
    ...
    if config.train.novelty_relabeling.enabled:
        assert config.train.preintv_relabeling.enabled

        novelty_config = config.train.novelty_relabeling
        ds_kwargs['model_ensemble_ckpts'] = novelty_config.model_ensemble_ckpts
        ds_kwargs['novelty_region'] = novelty_config.novelty_region
        ds_kwargs['relabel_strategy'] = novelty_config.relabel_strategy

    if config.train.classifier_relabeling.enabled:
        assert config.train.preintv_relabeling.enabled

        rlbling_config = config.train.classifier_relabeling
        ds_kwargs['classifier_ckpt'] = rlbling_config.ckpt_path
        ds_kwargs['relabeling_filter_key'] = rlbling_config.filter_key
        ds_kwargs['label_rollout_only'] = rlbling_config.label_rollout_only
        ds_kwargs['label_key'] = rlbling_config.label_key
        ds_kwargs['relabel_strategy'] = rlbling_config.relabel_strategy
        ds_kwargs['traj_label_type'] = rlbling_config.traj_label_type
        ds_kwargs['cls_threshold'] = rlbling_config.cls_threshold

        # TODO: pre-corl stuff
        # ds_kwargs['pseudo_labling'] = rlbling_config.pseudo_labling
        # ds_kwargs['demos_drop_th'] = rlbling_config.demos_drop_th
        # ds_kwargs['intv_drop_th'] = rlbling_config.intv_drop_th
        # ds_kwargs['preintv_drop_th'] = rlbling_config.preintv_drop_th
        #
        # ds_kwargs['rollout_demos_drop'] = rlbling_config.rollout_demos_drop
        # ds_kwargs['rollout_intvs_drop'] = rlbling_config.rollout_intvs_drop
        # ds_kwargs['rollout_preintvs_drop'] = rlbling_config.rollout_preintvs_drop

        # # uncomment for debugging with dummy classifier
        # ds_kwargs['debug'] = True

    if config.train.classifier_relabeling.enabled:
        dataset = ClassifierRelabeledDataset(**ds_kwargs)
    elif config.train.novelty_relabeling.enabled:
        dataset = NoveltyRelabeledDataset(**ds_kwargs)
    elif config.train.preintv_relabeling.enabled:
...
def run_epoch(model, data_loader, epoch, validate=False, num_steps=None):
    ...
    for _step in LogUtils.custom_tqdm(range(num_steps)):
        ...
        t = time.time()
        if isinstance(model, Dreamer) or isinstance(model, DreamerCombined):
    ...
    for i in range(len(step_log_all)):
        for k in step_log_all[i]:
            if k == "reconstructions" or k == "targets" or k == "reconstructions_zero":
                continue
    ...
    if confusion_matrix is not None:
        ...
        if not validate:
            wandb.log({"Conf Matrix/Train": wandb.Image(conf_matrix_fig)}, step=epoch)
        else:
            wandb.log({"Conf Matrix/Val": wandb.Image(conf_matrix_fig)}, step=epoch)

    # hack this for now
    # visualize reconstructed image and target image
    if "reconstructions" in step_log:
        reconstructions = step_log["reconstructions"]
        targets = step_log["targets"]
        for k in reconstructions:
            if "image" in k:
                #assert (reconstructions[k] >= 0).all()
                assert (targets[k] >= 0).all()
                #assert (reconstructions[k] <= 1).all()
                assert (targets[k] <= 1).all()

                wandb.log({"reconstructed image {}".format(k): wandb.Image(reconstructions[k])}, step=epoch)
                wandb.log({"target image {}".format(k): wandb.Image(targets[k])}, step=epoch)

    if "reconstructions_zero" in step_log:
        reconstructions = step_log["reconstructions_zero"]
        for k in reconstructions:
            if "image" in k:
                wandb.log({"reconstructed zero image {}".format(k): wandb.Image(reconstructions[k])}, step=epoch)
...
def _plot_confusion_matrix(conf_matrix):
    ...
    try:
        ax.xaxis.set_ticklabels(["normal", "failure", "intv"])
        ax.yaxis.set_ticklabels(["normal", "failure", "intv"])
    except:
        pass
