...
from robomimic.models.base_nets import Module, Sequential, MLP, ResidualMLP, RNN_Base, ResNet18Conv, SpatialSoftmax, \
...
class ObservationGroupEncoder(Module):
    ...
    def forward(self, **inputs):
        ...
        assert set(self.observation_group_shapes.keys()).issubset(
            inputs), "{} does not contain all observation groups {}".format(
...
class RNN_MIMO_MLP(Module):
    ...
    def __init__(
        ...
        per_step_net = None
        self._has_mlp = (len(mlp_layer_dims) > 0) or use_res_mlp
        if self._has_mlp:
            # intermediate MLP layers
            if use_res_mlp:
                self.nets["mlp"] = ResidualMLP(
                ...
                self.nets["mlp"] = MLP(
                    ...
                    activation=mlp_activation,
                    output_activation=mlp_activation, # make sure non-linearity is applied before decoder
                )

            self.nets["decoder"] = ObservationDecoder(
                decode_shapes=self.output_shapes,
                input_feat_dim=(res_mlp_kwargs['hidden_dim'] if use_res_mlp else mlp_layer_dims[-1]),
            )
            if self.per_step:
        ...
        self.stochastic_inputs = stochastic_inputs
        if stochastic_inputs:
            self.nets["stoch_layer"] = nn.Linear(rnn_input_dim, rnn_input_dim * 2) # mean and logvar
    ...
    def forward(self, rnn_init_state=None, return_state=False, return_obs_embedding=False, **inputs):
        """
        Args:
            inputs (dict): a dictionary of dictionaries with one dictionary per
                observation group. Each observation group's dictionary should map
        ...
        if return_state:
            outputs.update({"state": rnn_state})
        if return_obs_embedding:
            outputs.update({"obs_embedding": rnn_inputs})

        return outputs

    # helper function: only return encoded rnn inputs
    def forward_part1(self, **inputs):
        ...
        # add stochasticity to rnn inputs
        if self.stochastic_inputs:
            rnn_inputs = self.nets["stoch_layer"](rnn_inputs)

        return rnn_inputs

    # helper function: take in encoded rnn inputs; return policy outputs
    def forward_part2(self, rnn_inputs_dist, rnn_init_state=None, return_state=False):
        if self.stochastic_inputs:
            # rsample from rnn_inputs_dist
            rnn_inputs = self.zdistr(rnn_inputs_dist).rsample()
        else:
            rnn_inputs = rnn_inputs_dist

        assert rnn_inputs.ndim == 3  # [B, T, D]
        assert self.per_step # only support per_step for now

        outputs = self.nets["rnn"].forward(inputs=rnn_inputs, rnn_init_state=rnn_init_state, return_state=return_state)
        if return_state:
            outputs, rnn_state = outputs
            return {"model_outputs": outputs, "state": rnn_state}
        else:
...
class RNN_MIMO_MLP_HEAD(Module):
    """
    A wrapper class for a multi-step RNN and a per-step MLP and a decoder.
        ...
        Args:
            ...
            encoder_kwargs (dict or None): If None, results in default encoder_kwargs being applied. Otherwise, should
                ...
                obs_modality2: dict
                    ...
        """
        super(RNN_MIMO_MLP_HEAD, self).__init__()
        assert isinstance(input_obs_group_shapes, OrderedDict)
        assert np.all([isinstance(input_obs_group_shapes[k], OrderedDict) for k in input_obs_group_shapes])
        assert isinstance(output_shapes, OrderedDict)
        self.input_obs_group_shapes = input_obs_group_shapes
    ...
    def output_shape(self, input_shape):
        ...
        T = input_shape[obs_group][mod][0]
        TensorUtils.assert_size_at_dim(input_shape, size=T, dim=0,
                                       msg="RNN_MIMO_MLP_HEAD: input_shape inconsistent in temporal dimension")
        # returns a dictionary instead of list since outputs are dictionaries
        return {k: [T] + list(self.output_shapes[k]) for k in self.output_shapes}

    def forward(self, rnn_inputs, rnn_init_state=None, return_state=False):
        """
        Args:
            rnn_inputs: feature from previous embed encoder
