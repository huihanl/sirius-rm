...
from robomimic.models.obs_nets import MIMO_MLP, RNN_MIMO_MLP, RNN_MIMO_MLP_HEAD
from robomimic.models.vae_nets import VAE
from robomimic.models.distributions import TanhWrappedDistribution
from robomimic.models.distributions import MixtureSameFamily
...
class RNNActorNetwork(RNN_MIMO_MLP):
    ...
    def __init__(
        ...
        super(RNNActorNetwork, self).__init__(
            ...
            use_res_mlp=use_res_mlp,
            res_mlp_kwargs=res_mlp_kwargs,
            stochastic_inputs=stochastic_inputs,
        )
...
class RNNGMMActorNetwork(RNNActorNetwork):
    ...
    def forward_train(self, obs_dict, goal_dict=None, rnn_init_state=None, return_state=False):
        ...
        obs_embedding = RNN_MIMO_MLP.forward_part1(
            self, obs=obs_dict, goal=goal_dict)
        
        outputs = RNN_MIMO_MLP.forward_part2(self, rnn_inputs_dist=obs_embedding, 
                                             rnn_init_state=rnn_init_state, return_state=return_state)
        
        outputs["obs_embedding"] = obs_embedding

        model_outputs = outputs["model_outputs"]
        
        means = model_outputs["mean"]
        scales = model_outputs["scale"]
    ...
    def forward_embedding_only(self, obs_dict, goal_dict=None):
        ...
        Returns:
            ...
            rnn_state: return rnn state at the end if return_state is set to True
        """
        return RNN_MIMO_MLP.forward_part1(
            self, obs=obs_dict, goal=goal_dict)

    def forward_policy_only(self, obs_embedding, rnn_init_state=None):

        if len(obs_embedding.shape) == 2:
            obs_embedding = torch.unsqueeze(obs_embedding, 0)
        outputs = RNN_MIMO_MLP.forward_part2(self, rnn_inputs_dist=obs_embedding, 
                                             rnn_init_state=rnn_init_state, return_state=True)
        ...
        rnn_state = outputs["state"]

        return action[:, 0], rnn_state

    def forward_policy_only_train(self, obs_embedding, rnn_init_state=None):
    ...
    def forward(self, obs_dict, goal_dict=None, rnn_init_state=None, return_state=False, return_obs_embedding=False):
        """
        Samples actions from the policy distribution.
        ...
        if return_state and return_obs_embedding:
            return ad.sample(), outputs["state"], outputs["obs_embedding"]
        elif return_state:
            return ad.sample(), outputs["state"]
        return ad.sample()
    ...
    def forward_train_step(self, obs_dict, goal_dict=None, rnn_state=None):
        ...
        ad = MixtureSameFamily(
            mixture_distribution=mixture_distribution,
            component_distribution=component_distribution,
        )
        return ad, state

    def forward_step(self, obs_dict, goal_dict=None, rnn_state=None, return_obs_embedding=False):
        """
        ...
        if return_obs_embedding:
            acts, rnn_state, obs_embedding = self.forward(
                obs_dict, goal_dict, rnn_init_state=rnn_state, return_state=True, return_obs_embedding=return_obs_embedding)
            assert acts.shape[1] == 1
            return acts[:, 0], rnn_state, obs_embedding
        else:
            acts, rnn_state = self.forward(
                obs_dict, goal_dict, rnn_init_state=rnn_state, return_state=True, return_obs_embedding=return_obs_embedding)
            assert acts.shape[1] == 1
...
class RNNActorNetworkHEAD(RNN_MIMO_MLP_HEAD):
    """
    An RNN policy network that predicts actions from observations.
    """
    ...
    def __init__(
        ...
        output_shapes = self._get_output_shapes()
        super(RNNActorNetworkHEAD, self).__init__(
            input_obs_group_shapes=observation_group_shapes,
            output_shapes=output_shapes,
            mlp_layer_dims=mlp_layer_dims,
            mlp_activation=nn.ReLU,
    ...
    def output_shape(self, input_shape):
        ...
        T = input_shape[mod][0]
        TensorUtils.assert_size_at_dim(input_shape, size=T, dim=0,
                                       msg="RNNActorNetworkHEAD: input_shape inconsistent in temporal dimension")
        return [T, self.ac_dim]
    ...
    def forward(self, obs_dict, goal_dict=None, rnn_init_state=None, return_state=False):
        ...
        outputs = super(RNNActorNetworkHEAD, self).forward(
            rnn_inputs=rnn_inputs, rnn_init_state=rnn_init_state, return_state=return_state)
...
class RNNGMMActorNetworkHEAD(RNNActorNetworkHEAD):
    """
    An RNN GMM policy network that predicts sequences of action distributions from observation sequences.
    """
    ...
    def __init__(
        ...
        super(RNNGMMActorNetworkHEAD, self).__init__(
            obs_shapes=obs_shapes,
            ac_dim=ac_dim,
            mlp_layer_dims=mlp_layer_dims,
            rnn_hidden_dim=rnn_hidden_dim,
    ...
    def forward_train(self, rnn_inputs, goal_dict=None, rnn_init_state=None, return_state=False):
        """
        Return full GMM distribution, which is useful for computing
        quantities necessary at train-time, like log-likelihood, KL
        divergence, etc.
        ...
        Returns:
            ...
            rnn_state: return rnn state at the end if return_state is set to True
        """
        outputs = RNN_MIMO_MLP_HEAD.forward(
            self, rnn_inputs=rnn_inputs, rnn_init_state=rnn_init_state, return_state=return_state)
    ...
    def forward(self, rnn_inputs, goal_dict=None, rnn_init_state=None, return_state=False):
        """
        Samples actions from the policy distribution.
        ...
        Returns:
            action (torch.Tensor): batch of actions from policy distribution
        """
        out = self.forward_train(rnn_inputs=rnn_inputs, goal_dict=goal_dict, rnn_init_state=rnn_init_state,
    ...
    def forward_train_step(self, rnn_inputs, goal_dict=None, rnn_state=None):
        """
        Unroll RNN over single timestep to get action GMM distribution, which
        is useful for computing quantities necessary at train-time, like
        log-likelihood, KL divergence, etc.
        ...
        obs_dict = TensorUtils.to_sequence(obs_dict)
        ad, state = self.forward_train(
            rnn_inputs=rnn_inputs, goal_dict=goal_dict, rnn_init_state=rnn_state, return_state=True)
    ...
    def forward_step(self, rnn_inputs, goal_dict=None, rnn_state=None):
        """
        Unroll RNN over single timestep to get sampled actions.
        ...
        acts, state = self.forward(
            rnn_inputs, goal_dict=goal_dict, rnn_init_state=rnn_state, return_state=True)
        assert acts.shape[1] == 1
        return acts[:, 0], state
...
class VAE_MoMaRT(Module):
    ...
    def forward_train(self, obs_dict, goal_dict=None, freeze_encoder=False):
        ...
        inputs = {} # last image in a sequence for prediction
        for key in obs_dict:
...
class VAE_Goal(VAE_MoMaRT):
    ...
    def forward_train(self, obs_dict, goal_dict=None, freeze_encoder=False):
        ...
        outputs = {} # last image in a sequence for prediction
        for key in obs_dict:
            outputs[key] = obs_dict[key][:, -1, :]

        conditions = {} # conditions are current image on the first image in a sequence
        for key in obs_dict:
            conditions[key] = obs_dict[key][:, 0, :]

        inputs = obs_dict[key] # inputs are the entire sequence
...
class VAE_Action(Module):
    ...
    def __init__(
        ...
        Args:
            ...
            encoder_kwargs (dict or None): If None, results in default encoder_kwargs being applied. Otherwise, should
                ...
                obs_modality2: dict
                    ...
        """
        super(VAE_Action, self).__init__()
        ...
        self.conditioned_on_obs = conditioned_on_obs

        if not self.conditioned_on_obs:
    ...
    def decode(self, obs_dict=None, goal_dict=None, z=None, n=None):
        ...
        if not self.conditioned_on_obs:
            ...
            conditions = {} # conditioned on the first image in a sequence
            for key in obs_dict:
                conditions[key] = obs[key][:, 0, :]
                print("conditions key shape: ", conditions[key].shape)
    ...
    def forward_train(self, actions, obs_dict, goal_dict=None, freeze_encoder=False):
        ...
        actions = torch.reshape(actions, (action_shape[0], -1))
        action_inputs = OrderedDict(action=actions)
    ...
    def forward(self, actions, obs_dict, goal_dict=None, z=None):
        """
        Samples actions from the policy distribution.
        ...
        """
        # do the same with forward_train, for the error detection purpose
        return self.forward_train(actions, obs_dict)
